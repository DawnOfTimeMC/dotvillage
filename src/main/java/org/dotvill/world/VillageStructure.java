...
public class VillageStructure extends Structure {
    ...
    public void afterPlace(WorldGenLevel level, StructureManager structureManager, ChunkGenerator chunkGenerator, RandomSource random, BoundingBox boundingBox, ChunkPos chunkPos, PiecesContainer pieces) {
        createVillage(level.getLevel(), pieces);
    ...
    private void createVillage(ServerLevel level, PiecesContainer piecesContainer) {
        if (!this.addedVillage) {
            ModLogger.info("Adding a new village");
            Village village = VillageManager.addVillage(level, this.position, new org.dotvill.culture.Culture("dummy"));
