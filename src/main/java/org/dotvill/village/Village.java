...
public class Village {
    ...
    private void createOrLoadXpBar() {
        ...
        if (customBossEvents.get(ModUtils.resource(barID)) == null) {
            Component barText = Component.literal("Lutèce - Age II (Xp 150/1000)").withStyle(ChatFormatting.WHITE);
            this.villageXpBar = customBossEvents.create(ModUtils.resource(barID), barText);
            this.villageXpBar.setColor(BossEvent.BossBarColor.WHITE);
        } else {
            this.villageXpBar = customBossEvents.get(ModUtils.resource(barID));
        }
    }
    ...
    private void handleUnloadedVillagers() {
        List<Dillager> unloadedVillagers = new ArrayList<>();
        for (UUID villagerUUID : this.villagers) {
            if (this.level instanceof ServerLevel serverLevel) {
                Entity entity = serverLevel.getEntity(villagerUUID);
                if (entity instanceof Dillager villager && !serverLevel.isLoaded(villager.blockPosition())) {
                    unloadedVillagers.add(villager);
                }
            }
        }
        for (Dillager villager : unloadedVillagers) {
