...
public class VillageMap {
    ...
    private final BlockPos.MutableBlockPos villageNWCorner;
    private final BlockPos.MutableBlockPos villageSECorner;
    ...
    /**
     * Create the new instance of VillageMap, object use to position the building in Minecraft.
     * @param villageCenter The BlockPos of the center of the village, used to prioritize positions to put new buildings.
     */
    public VillageMap(BlockPos villageCenter){
        this.villageCenter = villageCenter;
        this.villageNWCorner = villageCenter.mutable();
        this.villageSECorner = villageCenter.mutable();
        this.villageMap = new int[1][1];
        this.init();
    }

    /**
     * Generate the basic architecture for a new village : 1+2 big paths forming a cross with 4 buds.
     */
    private void init(){
        // First let's put the vertical big path, with length of 2 * mini_size + big_width
        int halfBigPath = BIG_WIDTH / 2;
        Bud firstBud = Bud.createBud(this, this.villageCenter.offset(-halfBigPath, 0, -halfBigPath - DEFAULT_PATH_LENGTH), Corner.NORTH_WEST, new Direction[]{Direction.NORTH});
     ...
     * of the MapBuild. If the VillageMap is empty, and the MC Level allows the placement, the MapBuild is added to the map.
    ...
    /**
     * Function used to add new buildings in the village !
     * Tries to add the building in parameter.
     * @param building Building to be added in the village.
     */
    ...
    /**
     * Function used to add the new TownCenter in the village !
     * This will change the center of the village and the position of the new buildings.
     * @param building Building to be added in the village.
     */
    ...
    /**
     * @return This VillageMap map of builds.
     */
    ...
    /**
     * @return A new ID for a building. This ID is the number of building in the village + 1, because the first ID is 1.
     */
    ...
    /**
     * Put the given build in this VillageMap builds dictionary and update the VillageMap.
    ...
    public void addNewBuilds(int id, MapBuild build){
        ...
        this.updateVillageMap(build);
    }

    /**
     * Update the VilleMap by resizing it so that the new build fits, and add its ids in the VillageMap matrix.
     * @param build MapBuild that must be added or updated on the VillageMap.
     */
    protected void updateVillageMap(MapBuild build){
        this.resizeVillageMap(build);
     ...
     * @param build MapBuild that must be added or updated on the VillageMap.
     */
    private void resizeVillageMap(MapBuild build){
        int north = Math.max(0, this.villageNWCorner.getZ() - build.getOriginPos().getZ());
        int east = Math.max(0, build.getCornerPos(Corner.SOUTH_EAST).getX() - this.villageSECorner.getX());
        int south = Math.max(0, build.getCornerPos(Corner.SOUTH_EAST).getZ() - this.villageSECorner.getZ());
        int west = Math.max(0, this.villageNWCorner.getX() - build.getOriginPos().getX());
        if(north + east + south + west > 0){
            this.resizeVillageMap(north, east, south, west);
        }
    }
     ...
     */
    private void resizeVillageMap(int north, int east, int south, int west){
        this.villageNWCorner.move(-west, 0, -north);
        this.villageSECorner.move(east, 0, south);
        int rows = this.villageMap.length;
        int cols = this.villageMap[0].length;
        // Create the new matrix, Java fills it with 0 by default.
        int[][] newVillageMap = new int[north + rows + south][west + cols + east];
        // Copy the villageMap into the newVillageMap
        for(int i = 0; i < rows; i++) {
            System.arraycopy(this.villageMap[i], 0, newVillageMap[i + north], west, cols);
        }
        this.villageMap = newVillageMap;
    }

    /**
     * Add the given Bud in the VillageMap buds list.
    ...
    public void tryCreatePath(Bud bud){
        if(bud != null){
            ...
            //if(bud.getAdjacentPaths().length == 1 && VillageMapDisplay.RAND.nextFloat() < PATH_SPAWN_RATE){
    ...
    /**
     * Remove the given bud from this VillageMap Buds list. Called when a Bud is used to place a MapBuild.
     * @param bud Bud to be removed.
     */
     ...
     * @return True if the corresponding position in the VillageMap is empty.
     */
     ...
     * @return True if the corresponding position in the VillageMap is empty or contains the acceptedID.
     */
    ...
    /**
     * Returns the ID of the content of the block in the village map.
     * @param pos The real BlockPos of the block we study.
     * @return The ID of the content or 0 if the block has no building or is out of the VillageMap.
     */
    ...
    public int getIDInMapPos(int xMap, int zMap){
        int id = (xMap < 0 || zMap < 0 || zMap >= this.villageMap.length || xMap >= this.villageMap[0].length) ? 0 : this.villageMap[zMap][xMap];
        //TODO Improve the code below that was only useful to simulate Water
        return 1;//id == 0 && getSurfaceY(this, xMap, zMap) <= 55 ? -1 : id;
    }

    /**
     * Modify the VillageMap matrix by replacing the ID stored at the given coordinate.
     * @param xMap The coordinate x of the block in map coordinate.
     * @param zMap The coordinate z of the block in map coordinate.
    ...
    private void setIDInMapPos(int xMap, int zMap, int id){
        this.villageMap[zMap][xMap] = id;
    }

    /**
     * @param id Unique ID of the MapBuild instance in the VillageMap.
    ...
    /**
     * @return The BlockPos of the center of the village.
     */
    ...
    /**
     * @return The 2D array that describes the map of the village.
     */
    public int[][] getVillageMap(){
        return this.villageMap;
    }

    /**
     * @return The list of Buds currently available in the VillageMap.
     */
    ...
    /**
     * Replace the current center of the village with a new one. Used when the town center is built.
     * @param newCenter The new center of the village.
     */
    private void setVillageCenter(BlockPos newCenter){
        this.villageCenter = newCenter;
        //TODO Compute the new distance bud-center.
    }

    public int getMapX(int realX){
        return realX - this.villageNWCorner.getX();
    }

    public int getMapZ(int realZ){
        return realZ - this.villageNWCorner.getZ();
    }

    public BlockPos getVillageNWCorner(){
        return this.villageNWCorner.immutable();
    }
}
