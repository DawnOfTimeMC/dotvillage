...
import org.dotvill.registry.MotEntities;
import org.dotvill.trade.VillagerTradingModule;
import org.dotvill.village.Village;
import org.slf4j.Logger;

public class Dillager extends AgeableMob {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final EntityDataAccessor<Boolean> DATA_CROSSING_ARMS = SynchedEntityData.defineId(Dillager.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_READING = SynchedEntityData.defineId(Dillager.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Byte> DATA_CULTURE = SynchedEntityData.defineId(Dillager.class, EntityDataSerializers.BYTE);
    private static final EntityDataAccessor<Byte> DATA_PROFESSION = SynchedEntityData.defineId(Dillager.class, EntityDataSerializers.BYTE);
    private VillagerTradingModule tradingModule;
    ...
    public Dillager(EntityType<Dillager> entityType, Level level) {
        super(entityType, level);
        this.tradingModule = new VillagerTradingModule(this);
    }
    ...
    public void onFinalizeSpawnEvent() {
        setCulture(DillagerCulture.byBiome(level().getBiome(blockPosition())));
        setProfession(DillagerProfession.STONEMASON);
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_CULTURE, DillagerCulture.PLAINS.getId());
        this.entityData.define(DATA_PROFESSION, DillagerProfession.UNEMPLOYED.getId());
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (!level().isClientSide() && (hand == InteractionHand.MAIN_HAND)) {
            ...
            this.tradingModule.startTradingWithPlayer(player);
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        return MotEntities.DILLAGER.get().create(level);
    }
    ...
    public DillagerCulture getCulture() {return DillagerCulture.byId(this.entityData.get(DATA_CULTURE));}

    public void setCulture(DillagerCulture culture) {this.entityData.set(DATA_CULTURE, culture.getId());}

    public DillagerProfession getProfession() {return DillagerProfession.byId(this.entityData.get(DATA_PROFESSION));}

    public void setProfession(DillagerProfession profession) {this.entityData.set(DATA_PROFESSION, profession.getId());}
