package org.dotvill.entity.ai.vanillagoal;
...
import org.dotvill.construction.project.ConstructionProject;
...
import org.dotvill.entity.ai.systems.statemachine.State;
import org.dotvill.entity.ai.systems.statemachine.StateMachine;
...
public class BuilderWorkAI extends Goal {
    ...
    private final Dillager builder;
    ...
    public BuilderWorkAI(Dillager builder) {
    ...
    private boolean builderHasAdminOrderToBuild() {
        if (this.builder.hasAdminOrder() && (this.builder.getAdminOrder() instanceof AdminOrder.BuildOrder order)) {
            return ((order.getRequestedSite() != null ) && ConstructionProjectManager.get((ServerLevel)this.builder.level()).containsProject(order.getRequestedSite()));
        }
        return false;
    }
    ...
    private void setBuildingSite() {
        this.project = ((AdminOrder.BuildOrder)this.builder.getAdminOrder()).getRequestedSite();
    }
    ...
    public void tick() {
        if (!this.project.isCompleted()) {
            if (BuildingProjectCommand.showDebugInfo) {
                showDebugInfo();
            }
    ...
    private void moveToNextBuildPos() {
        ...
        if (this.builder.distanceToSqr(x, y, z) > (MAX_REACH_DIST / 2 )) {
            this.builder.getNavigation().moveTo(x, y, z, (BuildingProjectCommand.superBuilder ? 0.65F : BUILDER_SPEED ));
        } else {
            this.builder.getNavigation().stop();
        }
    }
    ...
    private void stopCheckingBuildingPlan() {
        this.nextPlanCheckCooldown = BuildingProjectCommand.superBuilder ? Integer.MAX_VALUE : this.builder.getRandom().nextInt(adjustedTickDelay((20 * 30)), adjustedTickDelay(20 * 60));
        this.builder.setItemInHand(InteractionHand.MAIN_HAND, ItemStack.EMPTY);
    ...
    public boolean requiresUpdateEveryTick() {
        return BuildingProjectCommand.superBuilder;
    }
