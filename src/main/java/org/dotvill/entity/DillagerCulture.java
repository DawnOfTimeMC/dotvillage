...
public enum DillagerCulture {
    DESERT, JUNGLE, PLAINS, SAVANNA, SNOW, SWAMP, TAIGA;

    private static final Map<ResourceKey<Biome>, DillagerCulture> BY_BIOME = Util.make(Maps.newHashMap(), (p) -> {
    ...
    public static DillagerCulture byId(byte id) {
        for(DillagerCulture culture : DillagerCulture.values()) {
            if (id == culture.getId()) {
                return culture;
            }
        }
    ...
    public static DillagerCulture byBiome(Holder<Biome> holder) {
