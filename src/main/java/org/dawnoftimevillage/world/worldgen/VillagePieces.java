...
public class VillagePieces {
    ...
    public static void addPieces(StructureTemplateManager manager, BlockPos villagePos, StructurePieceAccessor pieces, RandomSource random, VillageStructure.Culture culture) {
        ...
        List<BoundingBox> buildingsBoundingBoxes = Lists.newArrayList();
    ...
    public static class VillagePiece extends TemplateStructurePiece {
        public VillagePiece(StructureTemplateManager manager, ResourceLocation resourceLocation, BlockPos pos, Rotation rotation) {
            super(DoTVStructuresRegistry.VILLAGE_PIECE.get(), 0, manager, resourceLocation, resourceLocation.toString(), makeSettings(rotation), pos);
        }

        public VillagePiece(StructureTemplateManager manager, CompoundTag tag) {
            super(DoTVStructuresRegistry.VILLAGE_PIECE.get(), tag, manager, (p) -> makeSettings(Rotation.valueOf(tag.getString("Rot"))));
        }
